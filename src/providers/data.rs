...
impl YamlExtended {
    ...
    /// [`serde_yaml_ng::Value::apply_merge()`]. This method is _not_ intended to
    ...
    /// });
    /// ```
    pub fn from_str<'de, T: DeserializeOwned>(s: &'de str) -> serde_yaml_ng::Result<T> {
        let mut value: serde_yaml_ng::Value = serde_yaml_ng::from_str(s)?;
        value.apply_merge()?;
        T::deserialize(value)
    }
}
...
impl_format!(Yaml "YAML"/"yaml": serde_yaml_ng::from_str, serde_yaml_ng::Error);
impl_format!(Json "JSON"/"json": serde_json::from_str, serde_json::error::Error);
impl_format!(YamlExtended "YAML Extended"/"yaml": YamlExtended::from_str, serde_yaml_ng::Error);
